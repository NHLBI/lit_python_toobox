...
def l_2_opt(degree, l=0, L=1, weight=1, verbose=True):
  ...
  """
    ...
    if verbose:
      # Convert SymPy polynomial to a numerical function
      ...
      def eval_poly(res):
        if res.size == 1:
            return res[0]
        return res[0] + ax_x * eval_poly(res[1:])
    
      # Evaluate the error of the standard operation (AHA) and the polynomial preconditioned formulation
      iterations = degree + 1 # Iterations must be > degree + 1
      f_pgd = np.abs(1 - ax_x/L) ** (iterations) # Standard proximal gradient descent, normalized to L
      f_ppgd = np.abs(1 - eval_poly(res) * ax_x) # Polynomial preconditioned proximal gradient descent
      ...
      # Plot the same but for double the iterations (which is **2 due to power law, can sub in any float though)
      axs[1].plot(ax_x, 100 * f_pgd ** 2)
      axs[1].plot(ax_x, 100 * f_ppgd ** 2)

      axs[0].legend(["PGD", f"Poly. Precond. PGD (Degree {degree})"],
            fontsize=10,
            loc="upper right")
      axs[1].legend(["PGD", f"Poly. Precond. PGD (Degree {degree})"],
            fontsize=10,
            loc="upper right")

      axs[0].set_title(
          "A. Iteration Progress after $%d\;A^*A$ Evaluations" % (iterations), fontsize=12
      )
      axs[1].set_title(
          "B. Iteration Progress after $%d\;A^*A$ Evaluations" % (2*iterations), fontsize=12
      )
      for ax in axs.ravel():
        ax.grid()
        ax.set_xlabel(r"Spectrum of $A^*A$", fontsize=12)
        ax.set_ylabel("Error versus Iterate Limit (%)", fontsize=12)
        ax.set_ylim((0, 100))
      plt.show()
      ...
      print("> Degree:   %d" % degree)
      print("> Spectrum: [%0.2f, %0.2f]" % (l, L))
      print(f"> Time taken to compute polynomial: {toc - tic:.2f} seconds")
