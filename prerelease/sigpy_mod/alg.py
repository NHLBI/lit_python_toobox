    ...
    def _update(self):
        ...
        with self.device:
            ...
            if self.proxg is not None:
                ...
                print("Proximal operator being applied inside the GradientMethod() alg...")
            
            # Original method (FISTA)
            # if self.accelerate:
            #     t_old = self.t
            ...
            print("Joey is attempting Nesterov acceleration inside the GradientMethod() alg...")
            if self.accelerate:
                # **Nesterov Acceleration**
                a = 2.1
                k = self.iter
                step = k / (k + a + 1)
                backend.copyto(self.z, self.x + step * (self.x - x_old))
