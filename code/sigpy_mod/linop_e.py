...
def DLD(Linop, device=sp.Device(-1)):
    ...
    print(f"B1.idevice = {B1.idevice}; B1.odevice = {B1.odevice}")
    B2 = sp.linop.ToDevice(Linop.oshape, idevice=sp.Device(-1), odevice=device)
    print(f"B2.idevice = {B1.idevice}; B2.odevice = {B1.odevice}")
    Linop = B2.H*Linop*B1
    return Linop
...
def NFTs(ishape, coord, device=sp.Device(-1)):
    ...
    NFT = sp.linop.NUFFT(ishape[1:], coord=coord, oversamp=1.5, toeplitz=False)
    # NFTs = Diags([DLD(NFT, device=device)
    #              for i in range(n_Channel)], oshape, ishape)
    NFTs = Diags([NFT for i in range(n_Channel)], oshape, ishape) # TODO: see if removing device conversion (DLD) helps
